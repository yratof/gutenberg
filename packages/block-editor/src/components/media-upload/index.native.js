...
export default function withMediaUpload( WrappedComponent, getMediaType, onSelectURL ) {
	return class extends React.Component {
		getTakeMediaLabel() {
			const mediaType = getMediaType( this.props );
		...
		render() {
			...
			const onMediaLibraryButtonPressed = () => {
				requestMediaPickFromMediaLibrary( [ mediaType ], ( mediaId, mediaUrl ) => {
					if ( mediaId ) {
						onSelectURL( this.props, mediaId, mediaUrl );
					}
				} );
			};
			...
			return <WrappedComponent 
				getMediaOptionsPicker={ getMediaOptions }
				presentMediaOptions={ onPickerPresent }
				{...this.props }
			/>
		}
	}
